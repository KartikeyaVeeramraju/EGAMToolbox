%{
% FUNCTION:     spwmSinCoef

% DESCRIPTION:  returns the sin term coefficient of the Fourier expansion 
                of SPWM waveform
...
function value = spwmSinCoef(n,i,m10c, m10s, phi_hat, phi_bar)
yn = n*(pi/2)*sqrt(m10c^2 + m10s^2);
value = -(2/(n*pi))*sin(pi*(n+i)/2)*besselj(i,yn)*sin(n*phi_hat + i*phi_bar);
end